...
contract MockPriceOracle is IPriceOracle, AccessControl {
    ...
    function setPrice(address token, uint256 price) external onlyRole(Roles.PRICE_FEEDER_ROLE) {
        prices[token] = price;
    ...
    function getPrice(address token) external view override returns (uint256) {
        require(supportedTokens[token], "Price feed not supported");
        return prices[token];
    }
    ...
    function setLastUpdateTime(address token, uint256 timestamp) external onlyRole(Roles.PRICE_FEEDER_ROLE) {
        lastUpdateTimes[token] = timestamp;
    }

    function setSupportedStatus(address token, bool status) external onlyRole(Roles.PRICE_FEEDER_ROLE) {
        supportedTokens[token] = status;
    }

    function authorizeCaller(address caller) external onlyRole(Roles.ADMIN_ROLE) {
